...
void drawPolygonRotation(float angle)
{
	glBegin(GL_POLYGON); // Menggambar polygon
	glColor3f(0.0, 1.0, 0.0); // Warna hijau
	for (int i = 0; i < edges; i++)
	{
		float x = pntX[i] * cos(angle) - pntY[i] * sin(angle);
		float y = pntX[i] * sin(angle) + pntY[i] * cos(angle);
		glVertex2f(x, y);
	}
	glEnd();
	glFlush();
...
void drawPolygonTranslation(float tx, float ty)
{
	glBegin(GL_POLYGON); // Menggambar polygon
	glColor3f(1.0, 1.0, 0.0); // Warna kuning
	for (int i = 0; i < edges; i++)
	{
		glVertex2f(pntX[i] + tx, pntY[i] + ty);
	}
	glEnd();
	glFlush();
}

void drawPolygonScaling(float sx, float sy)
{
	glBegin(GL_POLYGON); // Menggambar polygon
	glColor3f(0.0, 0.0, 1.0); // Warna biru
	for (int i = 0; i < edges; i++)
	{
		glVertex2f(pntX[i] * sx, pntY[i] * sy);
	}
	glEnd();
	glFlush();
}
